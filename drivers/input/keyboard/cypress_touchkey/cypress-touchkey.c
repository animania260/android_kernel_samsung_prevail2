...
#define CYPRESS_IDAC_HOME	0X07
#define CYPRESS_IDAC_BACK	0X08
...
{
	struct cypress_touchkey_info *info = dev_info;
	u8 buf[] = {CYPRESS_AUTO_CAL, 0x00, 0x00, 0x01};
	int ret;
			 ...
			 const char *buf, size_t size)
{
	...
	while (retry--) {
		...
		dev_err(&info->client->dev,
			"[TOUCHKEY] Touchkey_update retry...\n");
	}
...
err_req_irq:
	gpio_free(info->pdata->gpio_led_en);
err_gpio_request:
