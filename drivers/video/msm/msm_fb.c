...
#define SAMSUNG_LOGO_HVGA "/samsunglogo_hvga.rle"
#define SAMSUNG_LOGO_WVGA "/samsunglogo_wvga.rle"
#endif
...
#ifdef CONFIG_FB_MSM_LOGO
	if (!charging_boot) {
		if (!msm_fb_pdata->detect_client("lcdc_s6d05a1_hvga")) {
			if (!load_565rle_image(SAMSUNG_LOGO_HVGA, bf_supported))
					;
		} else {
			if (!draw_rgb888_black_screen())
					;
		}
	} else {
			if (!draw_rgb888_black_screen())
