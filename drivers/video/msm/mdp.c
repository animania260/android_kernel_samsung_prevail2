...
#if !defined(CONFIG_FB_MSM_LCDC_S6D05A1_HVGA)
...
static int mdp_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_SAMSUNG_LCDC_DISPLAY
		/*
		* This part working for block underrun
		* when the time of boot up time.
		*/
		if (!firstBoot) {
			unsigned char *msm_mdp_ns;
			firstBoot = 1;

			/*disable LCDC Block*/
			outpdw(MDP_BASE + 0xc0000, 0x0);
			msleep(50);

			msm_mdp_ns = ioremap(0xab80014c, 4);
			/*Disable MDP clock*/
			outpdw(msm_mdp_ns, (inpdw(msm_mdp_ns) & 0x5FF));
			msleep(50);
		}
#endif
	...
	mfd->check_polarity = mdp_pdata->check_polarity;
#endif
