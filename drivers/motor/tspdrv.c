...
static ssize_t sysfs_MND_N_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d(%x)\n", g_nlra_gp_clk_n, g_nlra_gp_clk_n);
}

static ssize_t sysfs_MND_D_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d(%x)\n", g_nlra_gp_clk_d, g_nlra_gp_clk_d);
}

static ssize_t sysfs_MND_P_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d(%x)\n",
			g_nlra_gp_clk_PreDiv, g_nlra_gp_clk_PreDiv);
...
static DEVICE_ATTR(MND_N, 0664 , sysfs_MND_N_show, sysfs_MND_N_store);
static DEVICE_ATTR(MND_D, 0664 , sysfs_MND_D_show, sysfs_MND_D_store);
static DEVICE_ATTR(MND_P, 0664 , sysfs_MND_P_show, sysfs_MND_P_store);
static DEVICE_ATTR(MND_SET, 0664 , NULL, sysfs_MND_SET_store);
static DEVICE_ATTR(MND_CLK, 0664 , NULL, sysfs_MND_CLK_store);
...
static __devinit int tspdrv_probe(struct platform_device *pdev)
{
	...
	vib_clk = clk_get(NULL, "core_clk");
	if (IS_ERR(vib_clk))
		printk(KERN_ERR "[VIB] android vib clk failed!!!\n");
	else
