...
#if CONFIG_SEC_DEBUG
void set_restart_reason(unsigned reason)
{
...
#define RAMDUMP_MAGIC_NUM 0xCCCC
void kernel_secdebug_restart(void)
{
	int *smem_boot_reason_ptr = NULL;
	unsigned proc_comm_3 = ((unsigned)MSM_SHARED_RAM_BASE)+0x30;

	/*get the address where  AP restart reason to be stored*/
	smem_boot_reason_ptr = smem_alloc(SMEM_APPS_BOOT_MODE, (sizeof(int)));

	/* NULL check added */
	if (unlikely(NULL == smem_boot_reason_ptr))
		printk(KERN_ERR "AP resetting without setting the proper restart_reason\n");
	else
		/*write the AP restart reson*/
		writel_relaxed(restart_reason, smem_boot_reason_ptr);

	/* AP is ready to reboot. notify it to CP */
	writel_relaxed(0x01, proc_comm_3 + 0x04 /* status offset */);
	/*ensure that AP restart reason is written*/
	wmb();

	if (restart_reason != SECDEBUG_MODEM_UPLOAD_MODE) {
		/*the master cp or watchdog should reboot us*/
		msm_proc_comm_reset_modem_now();
		printk(KERN_EMERG "Secdebug reboot done from AP side\n");
	}
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	...
	if ((code == SYS_RESTART) && _cmd) {
		...
		if (!strcmp(cmd, "bootloader")) {
			...
			restart_reason = 0x12345671;
		} else if (!strncmp(cmd, "sud", 3)) {
			restart_reason = 0xabcf0000 | (cmd[3] - '0');
		} else if (!strncmp(cmd, "debug", 5) /* set debug_level */
